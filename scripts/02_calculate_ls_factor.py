...
#     # ensure numeric nodata for rasterio
...
#     )
#     # bounds -> destination CRS (older Rasterio has no 'precision' arg)
...
#         zi_m1 = z[i-1]; zi = z[i]; zi_p1 = z[i+1]  # noqa: F841 (doc clarity)
...
# # -----------------------------------------------------------------------------
# # NEW: Export concise LS stats to CSV (added)
# # -----------------------------------------------------------------------------
# def export_ls_stats_csv(ls_path, out_dir, logger, nodata=-9999):
#     """
#     Read the saved LS raster and write a CSV with descriptive stats:
#     count, min, p5, p25, median, p75, p95, max, mean, std.
#     """
#     try:
#         import numpy as np
#         import pandas as pd
...
#     dem_m, prof_m = reproject_raster(dem_ll, dem_profile_ll, dst_crs=metric_epsg, dst_res_m=90.0, resampling=Resampling.bilinear)
#     prof_m["nodata"] = prof_m.get("nodata", -9999.0)
...
#     ls_match = np.full(dem_ll.shape, -9999.0, dtype=np.float32)
#     warp.reproject(
...
#     )
#     ls_match = np.ma.masked_equal(ls_match, -9999.0)
...
#     ls_match = np.maximum(ls_match.filled(0.0), 0.0)
#     ls_match = np.ma.masked_invalid(ls_match)
...
#     save_geotiff(out_tif, ls_match.filled(-9999.0), out_prof)
...
#         view = np.clip(ls_match.filled(0.0), 0.0, DISPLAY_LS_CAP)
...
#     # --- NEW: Export concise stats CSV for reviewers (added) ---
#     export_ls_stats_csv(
#         ls_path=FACTORS_DIR / "ls_factor.tif",
#         out_dir=BASE_DIR / "outputs" / "statistics",
#         logger=logger
#     )
#     # ------------------------------------------------------------
