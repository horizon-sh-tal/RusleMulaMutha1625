...
# - Portable, ASCII-safe, no exotic deps.
# - Reads rain_total_YYYY.tif (mm) prepared earlier to *exact* DEM grid.
# - Applies configurable R = a * P^b (units documented in logs).
# - AOI masking to catchment polygon (if present).
# - Outputs per-year: GeoTIFF, PNG, and stats CSV + one master CSV.

# Expected inputs
# ---------------
# - DEM grid (master):      temp/dem_srtm_90m.tif
# - Annual rainfall totals: temp/factors/rain_total_YYYY.tif   (mm)
# - Optional AOI:           catchment/Mula_Mutha_Catchment.shp
...
# # Display only (PNG); data is never clipped
...
# # -----------------------------------------------------------------------------
# # Coefficients & config overrides
# # -----------------------------------------------------------------------------
# # Default coefficients for R = a * P^b, where:
# # P = annual rainfall (mm); R output unit depends on (a,b). Document in logs.
# R_COEFF_A = 0.35
# R_COEFF_B = 1.00
# R_FORMULA_NAME = "PowerLaw_R=a*P^b"
# R_OUTPUT_UNIT = "arbitrary-R (set via a,b)"  # override with your study units

# # Try importing project config to override paths/coeffs
# try:
#     sys.path.insert(0, str(SCRIPTS_DIR))
#     import config as _cfg  # type: ignore
#     # Paths (allow override)
...
# log_file = FACTORS_DIR / f"05_r_factor_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
# handlers = [
...
# def compute_r_from_p(p_mm, a, b, nodata_value=-9999.0):
#     """Compute R = a * P^b, keep nodata where p_mm is nodata or non-finite."""
#     out = np.full_like(p_mm, nodata_value, dtype=np.float32)
#     good = np.isfinite(p_mm) & (p_mm != nodata_value) & (p_mm >= 0.0)
#     if b == 1.0:
#         r = a * p_mm[good]
#     else:
#         r = a * np.power(p_mm[good], b, dtype=np.float64)
#     out[good] = r.astype(np.float32)
#     return out
...
#     if aoi_mask is None:
#         logger.info("AOI mask: not applied (shapefile missing or unreadable)")
#     else:
#         logger.info("AOI mask: applied (outside AOI -> nodata)")
...
#         # Reproject if needed (should already match, but we keep this robust)
...
#             outside = ~aoi_mask
#             p_match[outside] = -9999.0

#         # Compute R
#         r_arr = compute_r_from_p(p_match, a=R_COEFF_A, b=R_COEFF_B, nodata_value=-9999.0)

#         # Enforce nodata where DEM is nodata
#         if dem_prof.get("nodata", None) is not None:
#             dem_is_no = (dem == dem_prof["nodata"])
#             r_arr[dem_is_no] = -9999.0

#         # Stats
#         mask_valid = (r_arr != -9999.0) & np.isfinite(r_arr)
#         if mask_valid.any():
#             vals = r_arr[mask_valid]
#             stats = {
#                 "year": y,
#                 "min": float(vals.min()),
...
#                 "total_pixels": int(r_arr.size),
#                 "coverage_pct": float(vals.size / r_arr.size * 100.0),
#                 "formula": R_FORMULA_NAME,
#                 "a": float(R_COEFF_A),
#                 "b": float(R_COEFF_B),
#                 "unit_R": R_OUTPUT_UNIT
#             }
#             logger.info(f"R stats {y}: min={stats['min']:.3f}  mean={stats['mean']:.3f}  "
#                         f"median={stats['median']:.3f}  max={stats['max']:.3f}  "
#                         f"std={stats['std']:.3f}  n={stats['valid_count']} (coverage {stats['coverage_pct']:.2f}%)")
#         else:
