...
# 4) Rasterize AOI -> DEM grid; mask soil to AOI
...
# def reproject_to_match(src_arr, src_prof, match_prof, categorical=True, src_nodata=None, dst_nodata=-9999.0):
...
# def rasterize_aoi(match_prof):
#     """Rasterize AOI shapefile onto the DEM grid; returns boolean mask (True=inside)."""
...
#         logger.info(f"K stats (AOI-only): min={vmin:.4f}  mean={vmean:.4f}  median={vmed:.4f}  "
...
#         categorical=True, src_nodata=soil_nodata, dst_nodata=-9999.0
#     )
#     soil_match = np.rint(soil_match).astype(np.int16)  # ensure integer classes

#     # 4) AOI rasterization (DEM grid) and masking
#     aoi_mask = rasterize_aoi(dem_prof)
#     if aoi_mask is None:
#         logger.warning("AOI shapefile not found or invalid. Proceeding without AOI mask.")
...
#         soil_match, nodata_value=-9999.0,
...
#         k_raster[dem == dem_prof["nodata"]] = -9999.0
